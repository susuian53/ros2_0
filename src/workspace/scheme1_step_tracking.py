#!/usr/bin/env python3
"""
scheme1_step_tracking.py — 离散步态寻迹 (原始版)

航向使用路径 yaw + 横向偏差修正。
用法: python3 scheme1_step_tracking.py
"""
...
def run():
    ...
    last_x, last_y = x, y
    stuck_cnt = step = 0; total_dist = 0.0
    goal = path[-1]
    print(f"\n═══ Start ═══")
    ...
    while running:
        ...
        step += 1
        if step % 20 == 0:
            direction = "HIGH" if is_high else ("CROUCH" if is_crouch else ("BACK" if is_backward else ("CRV" if curve else "FWD")))
            print(f"  [{step:5d}] ({x:6.2f},{y:6.2f}) yaw={math.degrees(yaw):5.0f}°  "
                  f"wp={idx:5d}/{len(path)}  a_err={a_err:+5.1f}°  dg={dg:4.1f}m [{direction}]")
